...
namespace TreeTest
{
    public class Node<T> : IEqualityComparer, IEnumerable<T>, IEnumerable<Node<T>>
    {
        ...
        public void Add(Node<T> childNode, int index = -1)
        {
            ...
            //this.SetChildrenSiblingReferences();
        }
        ...
        //        cur.Previous = i != 0 ? this.children[i - 1] : null;
        //        cur.Next = i != this.children.Count - 1 ? this.children[i + 1] : null;
        //    }
        //}
        ...
        private void ForEach(Node<T> current, int relativeLevel, Action<Node<T>, int> action)
        {
            action(current, relativeLevel);
            relativeLevel++;

            foreach (var child in current.Children)
            {
                this.ForEach(child, relativeLevel, action);
            }
        }

        public void ForEach(Action<Node<T>, int> action)
        {
            this.ForEach(this, 0, action);
        }

        public string ToString(Func<Node<T>, string> toLineOfText, int indentation = 5)
        {
            var builder = new StringBuilder();
            this.ForEach((n, l) => builder.AppendLine($"{string.Empty.PadLeft(l * indentation)}{toLineOfText(n)}"));
